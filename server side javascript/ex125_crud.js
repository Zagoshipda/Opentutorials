    ...
    if(id){ //id 가 있는 경우-> 상세보기 -> 해당 data 항목이 가지는 모든 detail informatin을 가져와야함 -> select * 이 필요 -> 특정 id에 해당하는 모든 정보는 topic 변수가 가지고 있음
      ...
      conn.query(sql, [id], function(err, topic, fields){ //rows -> topic (이미 위쪽 query 콜백함수에서 topics 변수를 사용했으므로 다른 이름의 변수를 사용하기 위해 topic 사용)
        ...
        if(err){
          ...
          res.render('topic/view', {topics:topics, topic:topic[0]});  //글목록 보여주는 부분 : 일단 topics 는 sql data를 모두 가지고 있는 것.  topic 인자는 특정 하나의 id값에 대응하는 세부적인 data를 모두 담고 있는 (원소의 개수가 1개인)배열 이므로 그 중 첫 번째 원소를 가져온다는 의미에서 [0]을 사용
        }
      });
    }else{  //id 가 없는 경우 - home url
      res.render('topic/view', {topics:topics});  //view template file rendering
    }
